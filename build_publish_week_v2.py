...
def _verification_marker_html(ev: Dict[str, Any]) -> str:
    """HTML form of _verification_marker for the WordPress / Archive renderers.
    Wrapped in <em class="dc-unverified"> so it can be styled (or hidden) in CSS.
    Returns '' for verified events and for un-annotated appendices."""
    corr = ev.get("corroboration")
    tier = corr.get("verification_tier") if isinstance(corr, dict) else None
    if tier == "single_source_unverified":
        return ' <em class="dc-unverified">[single source]</em>'
    if tier == "unverified":
        return ' <em class="dc-unverified">[unverified]</em>'
    return ""


def _verification_marker_plain(ev: Dict[str, Any]) -> str:
    """Plaintext form of _verification_marker for the Scrivener appendix (no
    markup). Returns '' for verified events and un-annotated appendices."""
    corr = ev.get("corroboration")
    tier = corr.get("verification_tier") if isinstance(corr, dict) else None
    if tier == "single_source_unverified":
        return " [single source]"
    ...
    if isinstance(data, dict) and isinstance(data.get("categories"), list):
        for cat in data.get("categories") or []:
            ...
            for idx, ev in enumerate(events, start=1):
                ...
                label = " ".join(label_parts).strip()
                first_line = f"{idx}. {label}" if label else f"{idx}."
                marker = _verification_marker_plain(ev)
                if marker:
                    first_line = f"{first_line}{marker}"
                if summary:
                    first_line = f"{first_line}: {summary}"
...
def appendix_json_to_wordpress_html(
    *,
    ...
    def _render_event_li(ev: Dict[str, Any]) -> str:
        ...
        # Soft-annotation marker (raw HTML, appended AFTER esc(text) so it is
        # not escaped): flag events Step 9 could not verify.
        marker = _verification_marker_html(ev)

        # Optional URL, placed on a new line
        url = ev_field(ev, ["url", "URL", "link", "Link"], default="").strip()
        if url:
            return f"<li>{esc(text)}{marker}<br><a href=\"{esc(url)}\" target=\"_blank\" rel=\"noopener\">{esc(url)}</a></li>"
        return f"<li>{esc(text)}{marker}</li>"
