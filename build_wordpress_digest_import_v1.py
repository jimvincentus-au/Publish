...
# Column schema for digest-only WP All Import (kept compatible with WP All Export update mode)
...
def _predict_post_url(*, site_base_url: str, week_number: int, kind: str, title: str) -> str:
    ...
    Expected patterns (per site convention):
      ...
      - https://thedemocracyclock.com/week-49-digest-<slugified-title>/
    ...
    if not slug_title:
        return ""
    wk = int(week_number)
    kind_norm = str(kind or "narrative").strip().lower()
    if kind_norm not in {"appendix", "digest", "narrative"}:
        kind_norm = "narrative"
    prefix = f"week-{wk}-{kind_norm}-"
...
def setup_logger(level: str) -> logging.Logger:
    ...
    log_path = PUBLISH_LOGS_DIR / "build_digest_wordpress_import_v1.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build digest-only WP All Import XLSX from published Democracy Clock artifacts"
    )
...
def export_index(df: pd.DataFrame) -> Dict[Any, Dict[str, Any]]:
    """Index export rows by Week Number and optional Post Type.

    Compatibility behavior:
      - out[week] returns the first ``post`` row for that week (legacy single-row model)
      - out[(week, post_type)] returns the exact row for that post type when present
    """
    ...
    for _, r in df.iterrows():
        ...
        try:
            ...
            continue
        post_type = str(row.get("Post Type", "post") or "post").strip().lower() or "post"
        out[(wk, post_type)] = row
        if post_type == "post" and wk not in out:
            out[wk] = row
    return out
...
def _minutes_after_noon_to_time_str(minutes_after_noon: Optional[float | int]) -> str:
...
def _strip_first_paragraph_if_subtitle(html_text: str) -> str:
    """Remove the first short paragraph if it looks like a subtitle line."""
    if not html_text or '<p' not in html_text.lower():
        return html_text
    ...
    if inner and len(inner) <= 280:
        return html_text[:m.start()] + html_text[m.end():]
    return html_text


def _first_content_paragraph_from_html(html_text: str) -> str:
    """Return the first meaningful paragraph as plain text."""
    if not html_text:
        return ""
    for m in re.finditer(r"<p[^>]*>(.*?)</p>", html_text, flags=re.IGNORECASE | re.DOTALL):
        inner = m.group(1)
...
def _load_digest_metadata_for_week(week_number: int) -> Dict[str, Any]:
    """Load Step 8 digest metadata from Step 3 if available."""
    week_dir = Path(f"/Volumes/PRINTIFY24/Democracy Clock Automation/Step 3/Weeks/Week {week_number}")
    candidates = [
        week_dir / f"weekly_digest_metadata_stack_week{week_number}.json",
        week_dir / f"weekly_digest_spine_week{week_number}.json",
    ]
    for p in candidates:
        if p.exists():
            try:
...
def build_digest_post_row(
    *,
    ...
    """Build a digest import row (custom post type ``digest``)."""
    ...
    digest_meta = _load_digest_metadata_for_week(week_number)
    digest_html = digest_html_path.read_text(encoding="utf-8")
    wp_title = _derive_wp_title(
        week_number=week_number,
        kind="digest",
        metadata=digest_meta or metadata,
    ...
    if not wp_slug:
        wp_slug = f"week-{week_number}-digest-{_slugify_for_permalink(wp_title)}"
    if not wp_permalink:
        wp_permalink = _predict_post_url(
    ...
    excerpt = str(
        digest_meta.get("short_synopsis")
        or digest_meta.get("Short Synopsis")
        or digest_meta.get("short synopsis")
        or ""
    ).strip()

    dig_html = _strip_epigraph_blocks(digest_html)
    dig_html = _strip_first_h1(dig_html)
    dig_html = _strip_first_paragraph_if_subtitle(dig_html)

    if not excerpt:
        excerpt = _first_content_paragraph_from_html(dig_html)

    tags_raw = digest_meta.get("internal_tags") or digest_meta.get("InternalTags") or digest_meta.get("tags") or digest_meta.get("Tags") or []
    if isinstance(tags_raw, list):
        tags = ", ".join(str(x).strip() for x in tags_raw if str(x).strip())
    else:
...
def main() -> None:
    ...
    logger.info("Starting digest WordPress import build")
    logger.debug(f"Args: {args}")
    export_rows: Dict[Any, Dict[str, Any]] = {}
    ...
    for week in range(args.week, args.week + args.weeks):
        ...
        digest_html_path = week_dir / f"week{week:02d}-digest.html"
        if not digest_html_path.exists():
            msg = f"Missing digest HTML: {digest_html_path}"
            if args.skip_missing:
                logger.warning(msg + " (skipped)")
                continue
            raise FileNotFoundError(msg)
        ...
        rows.append(
            build_digest_post_row(
                ...
                logger=logger,
                existing=export_rows.get((week, "digest")),
            )
        )
        logger.info(f"Week {week:02d}: added digest import row from {digest_html_path.name}")
    ...
    if args.csv:
        ...
        logger.info(f"Wrote digest WordPress import CSV file: {csv_path}")
